...
from enrollment.models import Applicant
...
#
# ПРЕДСТАВЛЕНИЯ ДЛЯ ПРИКАЗОВ (Order)
#

@permission_required('orders.add_order', raise_exception=True)
def order_create(request):
    applicant_filter = ApplicantFilter(request.GET, queryset=Applicant.objects.filter(is_ready_for_enrollment=True, orders__isnull=True))
        ...
        if form.is_valid():
            ...
            if not selected_applicant_ids:
                ...
                return render(request, 'orders/order_form.html', {
                    'form': form, 'filter': applicant_filter,
                })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not selected_applicant_ids:
                messages.error(request, "Не выбрано ни одного абитуриента!")
                return render(request, 'orders/order_form.html', {
                    'form': form, 'filter': applicant_filter, 'object': order,
                })
            order = form.save()
            order.applicants.set(selected_applicant_ids)
...
class OrderListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    template_name = 'orders/order_list.html' # <-- Убедитесь, что этот путь правильный
...
def sign_order(request, pk):
    ...
    if order.status == 'draft':
        ...
        applicants_to_enroll = list(order.applicants.all())
